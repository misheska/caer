 ...
 * @param MODULE
 */
#define registerModuleClass(MODULE) caerModuleInfo caerModuleGetInfo() {return &(caer::ModuleStaticDefinitions<MODULE>::info);}
...
namespace caer {
    ...
    template<typename T> struct has_getName<T, void_t<
                                               decltype(T::getName()),
            enable_if_t<std::is_same<decltype(T::getName()), const char*>::value>
    >> : std::true_type {};
...
template<typename T> struct has_getDescription<T, void_t<
                                                  decltype(T::getDescription()),
...
template<typename T> struct has_getConfigOptions<T, void_t<
                                                    decltype(T::getConfigOptions(std::declval<std::map<std::string, caer::ConfigOption>&>()))
>> : std::true_type {};
...
template<typename T> struct has_inputStreamDefinition<T, void_t<
                                                         decltype(T::inputStreams),
...
template<typename T> struct has_outputStreamDefinition<T, void_t<
                                                          decltype(T::outputStreams),
 ...
 */
template <typename T> const size_t numberOfInputStreams = T::inputStreams == nullptr ? 0: CAER_EVENT_STREAM_IN_SIZE(T::inputStreams);

/**
 * Const that compiles to the number of output streams the provided module T has
 * @tparam T The module T to be analyzed
 */
template <typename T> const size_t numberOfOutputStreams = T::outputStreams == nullptr ? 0 : CAER_EVENT_STREAM_OUT_SIZE(T::outputStreams);
 ...
 * @tparam T The module T to be analyzed
 */
template <typename T> const caer_module_type moduleType = (numberOfInputStreams<T> == 0)
                                                          ? CAER_MODULE_INPUT
                                                          : ((numberOfOutputStreams<T> == 0)
                                                             ? CAER_MODULE_OUTPUT
...
class ModuleStaticDefinitions {
    ...
    static_assert(has_getDescription<T>::value, "Your module does not specify a `static const char* getDescription()` function."
                                                "This function should return a string with a description of the module.");
    static_assert(has_getConfigOptions<T>::value, "Your module does not specify a `static void getConfigOptions(std::map<std::string, caer::ConfigOption> &config)` function."
                                                  "This function should insert desired config options into the map.");
    static_assert(has_inputStreamDefinition<T>::value, "Your module does not have a `static const caer_event_stream_in inputStreams[];` property."
                                                       "This property should list the modules input streams.");
    static_assert(has_outputStreamDefinition<T>::value, "Your module does not have a `static const caer_event_stream_out outputStreams[];` property."
