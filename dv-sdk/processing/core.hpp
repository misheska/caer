...
namespace dv {
    ...
    public:
         ...
         */
        PartialEventData()
                : referencesConstData_(false), start_(0), length_(0),
   ...
   */
    ...
    public:
        ...
        : dataPartialsPtr_(dataPartialsPtr), offset_(0) {
        ...
        : dataPartialsPtr_(dataPartialsPtr), partialIndex_(partialIndex), offset_(offset) {}
    ...
    public:
        ...
        EventStore slice(size_t start, size_t length) const {
            ...
            size_t backSliceAmount = partialOffsets_[highIndex-1]
                                     + newPartials.back().getLength() - (start + length);
        ...
        EventStore sliceTime(time_t startTime, time_t endTime, size_t &retStart, size_t &retEnd) const {
            ...
            auto lowerPartial = std::lower_bound(dataPartials_.begin(), dataPartials_.end(), startTime,
                                                 PartialEventDataTimeComparator(false));
            auto upperPartial = std::upper_bound(dataPartials_.begin(), dataPartials_.end(), endTime,
                                                 PartialEventDataTimeComparator(true));
            ...
            if (newLength == 0) {
                retStart = isEmpty() ? 0 :
                           partialOffsets_[(size_t)(lowerPartial - dataPartials_.begin())];
                retEnd = retStart;
    ...
    class EventStreamSlicer {
        ...
        public:
             ...
             */
            SliceJob(const SliceType type, const size_t interval,
                    std::function<void(EventStore&)> callback)
            : type_(type), callback_(std::move(callback)),
            lastCallEndTime_(0), lastCallEnd(0) {
            ...
            void run(EventStore & packet) {
                ...
                if (type_ == TIME) {
                    ...
                    while (packet.getHighestTime() - lastCallEndTime_ >= timeInterval_) {
                        ...
                        EventStore slice = packet.sliceTime(lastCallEndTime_, lastCallEndTime_ + timeInterval_,
                                                             lastCallStart, lastCallEnd);
    ...
    public:
        ...
        explicit TimeMat(const cv::Size size) :
                data_(std::shared_ptr<time_t>(
                        new time_t[(unsigned long)(size.width * size.height)]{0},
                        [](const time_t *p) { delete[] p; })),
         ...
         */
        template <typename T>
        inline TimeMat operator+(const T &s) const  {
         ...
         */
        template <typename T>
        inline TimeMat& operator+=(const T &s) {
