...
namespace dv {
    ...
    private:
        ...
        void decay(coord_t x, coord_t y, time_t time) {
            ...
            switch (decayFunction_) {
                case LINEAR: {
                    potentialSurface_.at<float>(y, x) = (lastPotential >= neutralPotential_)
                ...
                case STEP: {
                    potentialSurface_.at<float>(y, x) = (double)(time - lastDecayTime) > decayParam_ ? neutralPotential_ : lastPotential;
                    break;
                }
                default: {
    ...
    public:
        ...
        Accumulator(const cv::Size &size, Accumulator::Decay decayFunction, double decayParam, bool synchronousDecay,
                                 float eventContribution, float maxPotential, float neutralPotential, float minPotential,
                                 bool rectifyPolarity) : rectifyPolarity_(rectifyPolarity),
                                                         eventContribution_(eventContribution), maxPotential_(maxPotential), neutralPotential_(neutralPotential),
                                                         minPotential_(minPotential), decayFunction_(decayFunction),  decayParam_(decayParam),
                                                         synchronousDecay_(synchronousDecay),
                                                         shape_(size),
                                                         decayTimeSurface_(TimeMat(size)),
                                                         potentialSurface_(cv::Mat(size, CV_32F, (double)neutralPotential)), highestTime_(0) {}
         ...
         */
        static Accumulator reconstructionFrame(const cv::Size &size, Decay decayFunction, double decayParam,
                float eventContribution, float neutralPotential) {
         ...
         * @return An accumulator with predefined settings
         */
        static Accumulator reconstructionFrame(const cv::Size &size, double tau,
                float eventContribution) {
            return Accumulator::reconstructionFrame(size, Accumulator::Decay::EXPONENTIAL, tau,
                    eventContribution, 0.0f);
        }
